...
static void mode_spinning_wheel(void)
{
  ...
  if (!SEGENV.allocateData(dataSize * strips))
    FX_FALLBACK_STATIC;
  ...
  struct virtualStrip
  {
    static void runStrip(uint16_t stripNr, uint32_t *state, bool settingsChanged, bool allReadyToRestart, unsigned strips)
    {
      ...
      if (needsReset && SEGMENT.check1)
      { // spin the spinner(s) only if the "Spin me!" checkbox is enabled
        state[CUR_POS_IDX] = 0;

        // Set velocity
        uint16_t speed = map(SEGMENT.speed, 0, 255, 300, 800);
        if (speed == 300)
        {                                                 // random speed (user selected 0 on speed slider)
        ...
        if (slowdown == 3000)
        { // random slowdown start time (user selected 0 on intensity slider)
...
static void mode_2D_lavalamp(void)
{
  ...
  if (!SEGENV.allocateData(sizeof(LavaParticle) * MAX_LAVA_PARTICLES))
    FX_FALLBACK_STATIC;
  ...
  if (currentNumParticles > MAX_LAVA_PARTICLES)
    currentNumParticles = MAX_LAVA_PARTICLES;
  ...
  for (int i = 0; i < MAX_LAVA_PARTICLES; i++)
  {
    if (!lavaParticles[i].active && hw_random8() < 32)
    { // spawn when slot available
      ...
      if (lavaParticles[i].size > MAX_BLOB_RADIUS)
        lavaParticles[i].size = MAX_BLOB_RADIUS;
  ...
  for (int i = 0; i < MAX_LAVA_PARTICLES; i++)
  {
    ...
    {
      if (p->vy >= 0)
      { // if going down, delay the particles so they won't go down immediately
      ...
      { // if going up, slow down the rise rate
        p->vy = -0.03f;
      }
    }
    ...
    {
      if (p->vy > 0)
      { // if going down, speed up the fall rate
        p->vy = 0.03f;
      }
      else if (p->vy <= 0)
      { // if going up, speed up the rise rate a little more
        p->vy = -0.05f;
      }
    }
    ...
    {
      if (p->vy > 0)
      { // if going down, speed up the fall rate a little more
        p->vy = 0.04f;
      }
      else if (p->vy <= 0)
      { // if going up, speed up the rise rate
        p->vy = -0.03f;
      }
    }
    ...
    {
      if (p->vy >= 0)
      { // if going down, slow down the fall rate
        p->vy = 0.02f;
      }
      else if (p->vy <= 0)
      { // if going up, delay the particles so they won't go up immediately
...
static void mode_2D_magma(void)
{
  ...
  if (MAGMA_MAX_PARTICLES < 2)
    FX_FALLBACK_STATIC;                    // matrix too narrow for lava bombs
  ...
  if (!shiftHue)
    FX_FALLBACK_STATIC; // safety check
...
static void mode_ants(void)
{
  ...
  for (int i = 0; i < numAnts; i++)
  {
    ...
    if (!passBy)
    {
      ...
      {
        if (fabsf(ants[j].velocity - ants[i].velocity) < 0.001f)
          continue; // Moving in same direction at same speed; avoids tiny denominators
...
static void build_morsecode_pattern(const char *morse_code, uint8_t *pattern, uint8_t *wordIndex, uint16_t &index, uint8_t currentWord, int maxSize)
{
  ...
  while (*c != '\0')
  {
    ...
    { // Must be a dash which is 3 pixels
      if (index >= maxSize - 3)
        return;
      SET_BIT8(pattern, index);
      wordIndex[index] = currentWord;
...
static void mode_morsecode(void)
{
  ...
  if (!SEGENV.allocateData(MORSECODE_PATTERN_BYTES + MORSECODE_WORD_INDEX_BYTES + MORSECODE_WORD_COUNT_BYTES))
    FX_FALLBACK_STATIC;
  ...
  if (needsRebuild)
  {
    ...
    for (char *c = text; *c; c++)
    {
      if (patternLength >= MORSECODE_MAX_PATTERN_SIZE - 10)
        break;
      ...
      else if (*c == ' ')
      {
        ...
        for (int x = 0; x < 4; x++)
        {
          if (patternLength >= MORSECODE_MAX_PATTERN_SIZE)
            break;
  ...
  for (int i = 0; i < SEGLEN; i++)
  {
    ...
    if (GET_BIT8(morsecodePattern, patternIndex))
    {
      ...
      if (SEGMENT.check1)
      { // make each word a separate color
