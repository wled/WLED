...
class DaliGearUsermod : public Usermod {
 private:
  ...
  static const char _enabled_key[];
    ...
    // ---------------------------------------------------------------------------
  static uint8_t busIsHigh() { return digitalRead(_rxPinStatic); }
  ...
  void handleCommand(uint8_t cmd) {
    switch (cmd) {
      case DALI_OFF: applyLevel(0); break;
      case DALI_UP:
        if (bri > 0) {
      ...
      case 0x90: {
        ...
        uint8_t status = ((bri > 0) ? 0x04u : 0x00u) | ((_daliAddr < 0) ? 0x40u : 0x00u);
      ...
      default: DEBUG_PRINTF("[DALI] unhandled command 0x%02x (%u) — ignored\n", cmd, cmd); break;
    }
  }
