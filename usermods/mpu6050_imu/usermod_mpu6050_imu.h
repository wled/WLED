...
// TODO: Fix sync: https://chatgpt.com/share/67b57fae-64fc-8002-9a3a-1aa7d9426dd4
...
class MPU6050Driver : public Usermod {
  private:
    // Variables to store the previous mode
    bool wasBlinkerActive = false;
    bool wasParkingActive = false;

    int prevEffect = 0;
    int prevBrightness = 255;
    bool prevState = true;
    const float alpha = 2.5; // Complementary filter constant
  ...
  public:
    ...
    // Global variables to track park mode timing
    //unsigned long parkStartTime = 0;
    #define stationaryGyroThreshold 0.1 // Adjust as per IMU sensitivity
    ...
    MPUData getMPUData() {
        ...
        // Forward/Backward detection using Z-axis and Y-axis rotation rate
        float accelZ = accel.acceleration.z; // Z-axis acceleration
        float gyroY = gyro.gyro.y;           // Y-axis angular velocity
        float gyroX = gyro.gyro.x;
        float gyroZ = gyro.gyro.z;
    
        MPUData data = { pitch, roll, accelZ, gyroY, gyroX, gyroZ };
        Serial.print("accel: ");
        Serial.print(accelZ);
        Serial.print(" -- gyroY: ");
        Serial.print(gyroY);
    ...
    int ridingThreshold = 0; //: Minimum pitch angle to consider forward/backward riding.
    int forwardAccelThreshold = 0; //: Z-axis acceleration threshold for forward movement.
    int backwardAccelThreshold = 0; //: Z-axis acceleration threshold for backward movement.
    int forwardGyroThreshold = 0; //: Positive gyroscope Y-axis rate for forward movement.
    int backwardGyroThreshold = 0; //: Negative gyroscope Y-axis rate for backward movement.
    int turnDetection = 20;
    int parkDetection = 15;
    // Global variables to track park mode timing
    unsigned long parkStartTime = 0;
    ...
    void loop() {
      static unsigned long turnStartTime = 0;  // Timer for detecting sustained turns
      static bool turning = false;             // Whether a turn is currently detected
      MPUData data = getMPUData();
      
      // ✅ Detect turning based on roll (for blinker activation)
      if (abs(data.roll) > turnDetection) {
          if (!turning) {
              // Start the timer when the threshold is first exceeded
              turning = true;
              turnStartTime = millis();
          }
    
          // Check if the turn has been sustained for more than 500 ms
          if (millis() - turnStartTime > 500) {
              Serial.println("Blinker enabled");
    
              // Enable the correct blinker based on roll direction
              leftBlinkerActive = (data.roll > 0);   // Left turn
              rightBlinkerActive = (data.roll < 0);  // Right turn
    
              // Activate the blinker effect (replace existing mode)
              if (!wasBlinkerActive) {
                  wasBlinkerActive = true;
                  prevEffect = strip.getMainSegment().mode;  // Store previous effect
                  strip.getMainSegment().setMode(FX_MODE_BLINKER);
              }
          }
      } else {
          // Reset the timer if the roll drops below the threshold
          turning = false;
          
          // Restore previous mode when turning stops
          if (wasBlinkerActive) {
              Serial.println("Blinker disabled");
              wasBlinkerActive = false;
              leftBlinkerActive = false;
              rightBlinkerActive = false;
              strip.getMainSegment().setMode(prevEffect); // Restore previous effect
          }
      }
      // ✅ Detect parking based on pitch and no motion
      if (abs(data.pitch) > parkDetection) {
        // Check if IMU is stationary
        if (abs(data.gyroX) < stationaryGyroThreshold &&
            abs(data.gyroY) < stationaryGyroThreshold &&
            abs(data.gyroZ) < stationaryGyroThreshold) {
            

            if (!isStationary) {
                isStationary = true;  // Start stationary timer
                parkStartTime = millis();
            }

            // Check if the stationary condition has been held for over 1 second
            if (millis() - parkStartTime > 1000) {
              Serial.println("Parking enabled");
            }
        } else {
            // Reset the stationary timer if the IMU starts moving again
            isStationary = false;
        }
    }
  }
