...
static void activateNightMode_visualremote() {
...
static bool resetNightMode_visualremote() {
...
static void brightnessUp_visualremote() {
  if (nightModeActive_visualremote()) return;
...
static void brightnessDown_visualremote() {
  if (nightModeActive_visualremote()) return;
...
static void setOn_visualremote() {
...
static void setOff_visualremote() {
  resetNightMode_visualremote();
  if (bri) {
    toggleOnOff();
    stateUpdated(CALL_MODE_BUTTON);
...
void presetWithFallback_visualremote(uint8_t presetID, uint8_t effectID, uint8_t paletteID) {
  resetNightMode_visualremote();
  applyPresetWithFallback(presetID, CALL_MODE_BUTTON_PRESET, effectID, paletteID);
}

static bool remoteJson_visualremote(int button) {
  char objKey[10];
  bool parsed = false;
...
void handleRemote_visualremote(uint8_t *incomingData, size_t len) {
