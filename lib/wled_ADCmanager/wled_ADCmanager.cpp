 ...
 *
 * calling begin() will start sampling an ADC pin in continuous mode
 * any subsequent call for analogRead() or analogReadMilliVolts() will stop the continuous sampling immediately,
 ...
 * replaces analogRead() and analogReadMilliVolts() with managed functions for code compatibility
 ...
 - need to add a "buffer full" callback?
 - add namespace?
 */
...
#define DMA_BLOCKSIZE 128 // DMA buffer block size, IDF driver uses 5 blocks under the hood, there is an ISR call each time a block finishes so dont make it too small
...
bool WLEDAdcManager::_initContinuousADC() {
  ...
  adc_continuous_handle_cfg_t hcfg = {
    ...
    .conv_frame_size    = DMA_BLOCKSIZE, // use fixed DMA buffer size of 256 bytes (ADC driver creates 5 DMA descriptors with one buffer each, at 20kHz this means an interrupt every 1.4ms
...
void WLEDAdcManager::_drainToCache() {
  ...
  adc_continuous_stop(_handle); // stop after reading (if stopped before, no samples are being read)
}
...
bool WLEDAdcManager::readSamples(int16_t* buffer, uint16_t numSamples, uint32_t timeoutMs) {
  ...
  const int tmpBfrSize = 128; // use buffer on stack to read samples from the driver
  ...
  while (out < numSamples) {
    ...
    // TODO:  add the possibility to "read what is available" and return the number of samples read?
    if (err != ESP_OK || n == 0) {
      // something went wrong, re-init the continuous transfer
      DEBUG_PRINTF_P(PSTR("ADC read error %d, got n=%d restarting"),err,n);
      _endContinuousADC();  // tear it down
      _initContinuousADC(); // re-init and start sampling
      xSemaphoreGive(_mutex);
      return false;
    }
    ...
    for (uint16_t i = 0; i < got; i++) {
      buffer[out++] = (int16_t)(temp[i].WLED_ADC_OUT_TYPE.data); // TODO: need to check bit width? see not above
    }
  }

  //adc_continuous_flush_pool(_handle); // flush remaining data -> no need, just let the samples accumulate
