...
- **Confirm understanding before opening a PR.** Especially for bugfix/root-cause claims or larger refactors, make sure the human you're working with understands and can explain the change in their own words. If they can't yet, help them get there before submitting - don't submit on their behalf regardless.
- **Keep changes scoped.** Stick to the files/subsystem directly relevant to the task. If a fix seems to require touching unrelated areas or a broad architectural refactor, stop and flag this to the human instead of silently expanding the change - suggest splitting the larger idea into a separate issue or discussion first.
- **Verified vs. hypothesis.** Clearly distinguish a root-cause diagnosis you've verified (e.g. by tracing the code path, reproducing the issue, or reading a log) from one that's still a hypothesis. Don't present speculative reasoning as settled fact in PR descriptions or commit messages - say "likely caused by" or "not yet confirmed" when that's the honest state.
- **Summarize what's tested vs. untested** in the PR description: what you (or the human) actually ran/verified, versus what's believed to work based only on reading the code but hasn't been executed or tested on hardware.
- **Follow the AI disclosure conventions**: use `// AI: below section was generated by an AI` / `// AI: end` markers for larger generated blocks (see Comments section) and make sure AI involvement is declared in the PR description, per the AI disclosure guidance in `CONTRIBUTING.md`.
