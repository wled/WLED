...
// Free the notifier snapshot after completion, failure, or replacement.
...
static void serviceEspNowSyncTx() {
  ...
  // Packet zero carries global state plus segments; later packets carry only segments.
...
// Snapshot udpOut because notify() uses a stack buffer that cannot outlive this call.
static void queueEspNowSyncTx(const uint8_t* payload, size_t segmentCount) {
  ...
  // A newer notification supersedes any snapshot still being transmitted.
  resetEspNowSyncTx();
  ...
  // The first packet has less segment room because it also contains global state.
...
void espNowSentCB(uint8_t* address, uint8_t status) {
  ...
  // Delivery failures stop a stale live subscriber before its lease expires.
...
void espNowReceiveCB(uint8_t* address, uint8_t* data, uint8_t len, signed int, bool broadcast) {
  ...
  // Validate fragment shape before copying data supplied by the radio.
  ...
  if (buffer->packet == 0) {
    // Packet zero always starts a new sync snapshot.
