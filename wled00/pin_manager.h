...
/*
 * Allows PinManager to "bloqueo" an allocation to a specific
 * propietario, so someone else doesn't accidentally de-allocate
...
enum struct PinOwner : uint8_t {
  ...
  // Use Usermod IDs from constante.h here
  UM_Unspecified       = USERMOD_ID_UNSPECIFIED,        // 0x01
  UM_Example           = USERMOD_ID_EXAMPLE,            // 0x02 // Usermod "usermod_v2_example.h"
  UM_Temperature       = USERMOD_ID_TEMPERATURE,        // 0x03 // Usermod "usermod_temperature.h"
  // #definir USERMOD_ID_FIXNETSERVICES                  // 0x04 // Usermod "usermod_Fix_unreachable_netservices.h" -- Does not allocate pins
  ...
  // #definir USERMOD_ID_AUTO_SAVE                       // 0x09 // Usermod "usermod_v2_auto_save.h" -- Does not allocate pins
  // #definir USERMOD_ID_DHT                             // 0x0A // Usermod "usermod_dht.h" -- Statically allocates pins, not compatible with pinManager?
  // #definir USERMOD_ID_VL53L0X                         // 0x0C // Usermod "usermod_vl53l0x_gestures.h" -- Uses "estándar" HW_I2C pins
  ...
  // #definir USERMOD_ID_RTC                             // 0x0F // Usermod "usermod_rtc.h" -- Uses "estándar" HW_I2C pins
  // #definir USERMOD_ID_ELEKSTUBE_IPS                   // 0x10 // Usermod "usermod_elekstube_ips.h" -- Uses quite a few pins ... see Hardware.h and User_Setup.h
  // #definir USERMOD_ID_SN_PHOTORESISTOR                // 0x11 // Usermod "usermod_sn_photoresistor.h" -- Uses hard-coded pin (PHOTORESISTOR_PIN == A0), but could be easily updated to use pinManager
...
namespace PinManager {
  ...
  // Allocates a single pin, with an propietario etiqueta.
  // De-allocation requires the same propietario etiqueta (or anular)
  bool allocatePin(byte gpio, bool output, PinOwner tag);
  // Allocates all the pins, or allocates none of the pins, with propietario etiqueta.
  // Provided to simplify error condición handling in clients
  // usando more than one pin, such as I2C, SPI, rotary encoders,
  // ethernet, etc..
  ...
  // will retorno verdadero for reserved pins
  bool isPinAllocated(byte gpio, PinOwner tag = PinOwner::None);
  // will retorno falso for reserved pins
