...
void IRAM_ATTR BusDigital::setPixelColor(unsigned pix, uint32_t c) {
  ...
  if (_type == TYPE_WS2812_RGBW_PAIR) {
    ...
    // Map one logical RGBW bulb onto two WS281x pixels observed as WW first, RGB second.
...
uint32_t IRAM_ATTR BusDigital::getPixelColor(unsigned pix) const {
  ...
  if (_type == TYPE_WS2812_RGBW_PAIR) {
    // AI: below section was generated by an AI
    // Reconstruct one logical RGBW value from the paired WW and RGB physical pixels.
    unsigned physicalPix = (pix + _skip) * 2;
    const uint8_t co = _colorOrderMap.getPixelColorOrder(pix+_skip+_start, _colorOrder) & 0x0F;
