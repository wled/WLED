...
BusHub75Matrix::BusHub75Matrix(const BusConfig &bc) : Bus(bc.type, bc.start, bc.autoWhite) {
  ...
  _isVirtual = false;
  _isVirtualScan = false;
  // aliases for easier reading
  unsigned panelWidth  = bc.pins[0];
  unsigned panelHeight = bc.pins[1];
  unsigned chainLength = bc.pins[2];
  _rows = (bc.pins[3] == 0 || bc.pins[3] > 16) ? 1 : bc.pins[3];
  _cols = (bc.pins[4] == 0 || bc.pins[4] > 16) ? 1 : bc.pins[4];
  ...
  if (bc.type == TYPE_HUB75MATRIX_HS) {
      ...
      // The DMA driver emits two RGB rows in parallel. Its 40-pixel virtual-panel remap converts this 160x20 surface to one 80x40, 1/20-scan panel.
      _isVirtual = true;
      mxconfig.mx_width = min(128U, panelWidth) * 2;
      mxconfig.mx_height = min(64U, panelHeight) / 2;
  ...
  _isVirtualScan = (bc.type == TYPE_HUB75MATRIX_QS || bc.type == TYPE_HUB75MATRIX_20S);
  ...
  // Set via platformio.ini: -D WLED_HUB75_COLOR_DEPTH=3
  #ifdef WLED_HUB75_COLOR_DEPTH
    mxconfig.setPixelColorDepthBits(WLED_HUB75_COLOR_DEPTH);
    DEBUGBUS_PRINTF_P(PSTR("HUB75 colour depth override: %u bits\n"), (unsigned)WLED_HUB75_COLOR_DEPTH);
  #else
  // AI: end
  ...
  else if (totalPixels >= 1024) mxconfig.setPixelColorDepthBits(5);
  else if (mxconfig.mx_height >= 64) {
    if (totalPixels > 192) mxconfig.setPixelColorDepthBits(3);
    else if (totalPixels > 64)  mxconfig.setPixelColorDepthBits(4);
...
//  HUB75_I2S_CFG::i2s_pins _pins={R1_PIN, G1_PIN, B1_PIN, R2_PIN, G2_PIN, B2_PIN, A_PIN, B_PIN, C_PIN, D_PIN, E_PIN, LAT_PIN, OE_PIN, CLK_PIN};
  ...
  #endif
  #if defined(WLED_HUB75_80X40_ICN2038P_PINOUT)
  // ESP32 DevKit V1 wiring for the 80x40, 1/20-scan ICN2038P panel.
  DEBUGBUS_PRINTLN("MatrixPanel_I2S_DMA - WLED 80x40 ICN2038P pinout");
  mxconfig.gpio = { 25, 26, 27, 14, 12, 13, 23, 19, 5, 17, 32, 4, 15, 16 };

  #elif defined(ESP32_FORUM_PINOUT) || defined(FORUM_ESP32_PINOUT) // Common format for boards designed for SmartMatrix
...
*/
  ...
  */
  ...
  // AI: below section was generated by an AI
  // For virtual-scan types (QS, 20S) with multiple chained panels, also promote chainType so
  // VirtualMatrixPanel can translate pixel coordinates across panel boundaries.
  if (_isVirtualScan && mxconfig.chain_length > 1 && chainType == CHAIN_NONE) {
  ...
  if (chainType != CHAIN_NONE || _isVirtualScan) {
    _isVirtual = true;
  ...
  if (_isVirtual) {
    ...
    if (!virtualDisp) { // catch alloc error
      ...
      if (bc.type == TYPE_HUB75MATRIX_QS) {
        ...
        // Two or more 80x40 panels may be chained to create a wider (160x40, 240x40, ...) display.
        if (panelWidth != 80 || panelHeight != 40) {
          DEBUGBUS_PRINTLN(F("HUB75 1/20 ICN2038P: each panel must be 80x40"));
          cleanup();
          return;
        }
        // AI: end
        virtualDisp->setPhysicalPanelScanRate(FOUR_SCAN_40_80PX_HFARCAN);
      }
    }
  }
...
std::vector<LEDType> BusHub75Matrix::getLEDTypes() {
  return {
    ...
    {TYPE_HUB75MATRIX_20S,    "H",     PSTR("HUB75 (1/20 Scan, ICN2038)")},
  };
}

size_t BusHub75Matrix::getPins(uint8_t* pinArray) const {
  if (pinArray) {
    pinArray[0] = _isVirtualScan ? mxconfig.mx_width  /2 : mxconfig.mx_width;
    pinArray[1] = _isVirtualScan ? mxconfig.mx_height *2 : mxconfig.mx_height;
