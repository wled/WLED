...
void BusDigital::estimateCurrent() {
  ...
  uint16_t standbyMilliamps = getLength(); // add 1mA standby current per LED to total (WS2812: ~0.7mA, WS2815: ~2mA)
...
void BusDigital::applyBriLimit(uint8_t newBri) {
  ...
  if (newBri == 0) {
    ...
    const uint16_t standbyMilliamps = (_type == TYPE_WS2812_RGBW_PAIR) ? getLength()*2 : getLength(); // each LED uses about 1mA in standby
...
void IRAM_ATTR BusDigital::setPixelColor(unsigned pix, uint32_t c) {
  ...
  if (_type == TYPE_WS2812_RGBW_PAIR) {
    ...
    const uint8_t co = _colorOrderMap.getPixelColorOrder(pix+_skip+_start, _colorOrder);
    PolyBus::setPixelColor(_busPtr, _iType, physicalPix, RGBW32(W(c), W(c), W(c), 0), co);
    PolyBus::setPixelColor(_busPtr, _iType, physicalPix + 1, RGBW32(R(c), G(c), B(c), 0), co);
    return;
  }
  pix += _skip;
...
uint32_t IRAM_ATTR BusDigital::getPixelColor(unsigned pix) const {
  ...
  if (_type == TYPE_WS2812_RGBW_PAIR) {
    unsigned physicalPix = (pix + _skip) * 2;
    const uint8_t co = _colorOrderMap.getPixelColorOrder(pix+_skip+_start, _colorOrder);
    uint32_t ww = restoreColorLossy(PolyBus::getPixelColor(_busPtr, _iType, physicalPix, co), _NPBbri);
    uint32_t rgb = restoreColorLossy(PolyBus::getPixelColor(_busPtr, _iType, physicalPix + 1, co), _NPBbri);
