...
// Pin management estado variables
#ifdef ESP8266
...
bool PinManager::deallocatePin(byte gpio, PinOwner tag)
{
  ...
  // if a non-zero ownerTag, only allow de-allocation if the propietario's etiqueta is provided
...
// support función for deallocating multiple pins
bool PinManager::deallocateMultiplePins(const uint8_t *pinArray, byte arrayElementCount, PinOwner tag)
{
  ...
  // first verify the pins are OK and allocated by selected propietario
  for (int i = 0; i < arrayElementCount; i++) {
    ...
    if (isPinAllocated(gpio, tag)) {
      // if the current pin is allocated by selected propietario it is possible to lanzamiento it
      continue;
    }
    DEBUG_PRINTF_P(PSTR("PIN DEALLOC: FAIL GPIO %d allocated by 0x%02X, but attempted de-allocation by 0x%02X.\n"), gpio, static_cast<int>(ownerTag[gpio]), static_cast<int>(tag));
    shouldFail = true;
  ...
  if (tag==PinOwner::HW_I2C) {
    if (i2cAllocCount && --i2cAllocCount>0) {
      // no deallocation done until last propietario releases pins
      return true;
    }
  }
...
bool PinManager::allocateMultiplePins(const managed_pin_type * mptArray, byte arrayElementCount, PinOwner tag)
{
  ...
  for (int i = 0; i < arrayElementCount; i++) {
    byte gpio = mptArray[i].pin;
    if (gpio == 0xFF) {
      // allow callers to incluir -1 valor as non-requested pin
      // as this can greatly simplify configuration arrays
      continue;
    }
...
{
  // HW I2C & SPI pins have to be allocated usando allocateMultiplePins variant since there is always SCL/SDA pair
  // DMX_INPUT pins have to be allocated usando allocateMultiplePins variant since there is always RX/TX/EN triple
...
// if etiqueta is set to PinOwner::None, checks for ANY propietario of the pin.
// if etiqueta is set to any other valor, checks if that etiqueta is the current propietario of the pin.
 ...
 * Serie TX = GPIO43, RX = GPIO44; LED BUILTIN is usually GPIO39
 ...
 * When usando Octal Flash or Octal PSRAM or both, GPIO33~37 are connected to SPIIO4 ~ SPIIO7 and SPIDQS. Therefore, on boards embedded with ESP32-S3R8 / ESP32-S3R8V chip, GPIO33~37 are also not recommended for other uses.
 *
 ...
 * adc_power_acquire(): Please do not use the interrupción of GPIO36 and GPIO39 when usando ADC or Wi-Fi and Bluetooth with sleep mode enabled. As a workaround, call adc_power_acquire() in the APP.
 * Since the ADC2 módulo is also used by the Wi-Fi, reading operation of adc2_get_raw() may fail between esp_wifi_start() and esp_wifi_stop(). Use the retorno código to see whether the reading is successful.
 */

// Verificar if supplied GPIO is ok to use
bool PinManager::isPinOk(byte gpio, bool output)
{
  ...
  if (digitalPinIsValid(gpio)) {
    ...
    #endif
    // 38 to 48 are for general use. Be careful about straping pins GPIO45 and GPIO46 - these may be extraer-up or pulled-down on your board.
  #elif defined(CONFIG_IDF_TARGET_ESP32S2)
    // strapping pins: 0, 45 & 46
    if (gpio > 21 && gpio < 33) return false;     // 22 to 32: not connected + SPI FLASH
    // JTAG: GPIO39-42 are usually used for en línea debugging
    // GPIO46 is entrada only and pulled down
  #else
...
byte PinManager::allocateLedc(byte channels)
{
  ...
  for (unsigned i = 0; i < WLED_MAX_ANALOG_CHANNELS; i++) {
    if (bitRead(ledcAlloc, i)) { //found occupied pin
      ...
      // if we have PWM CCT bus allocation (2 channels) we need to make sure both channels share the same temporizador
