...
bool RmtBus::begin() {
  ...
  config.flags.invert_out = _inverted ? 1 : 0; // hardware signal inversion via GPIO matrix (replaces esp_rom_gpio hack); applied at channel creation, setInverted() must be called before begin()
...
void RmtBus::end() {
  ...
  // Free encode buffer (also freed by ~PixelBus; free() is compatible with heap_caps_malloc)
