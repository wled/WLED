...
public: 
  ...
  // Memory estimation: per-bus encode buffer + shared LCD DMA context (on first bus)
  static size_t estimateMemory(uint16_t numPixels, uint8_t channelCount, bool isFirstBus = true) {
    ...
    if (isFirstBus) mem += WLEDPB_LCD_DMA_BUFFER_SIZE * 2 + 64;  // shared: 2× DMA buffers + 2× descriptors
    return mem;
  }
