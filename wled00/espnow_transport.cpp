...
// The callback writes these before publishing sentEventPending.
...
// Reset indices under the same lock used by the receive callback.
...
// Copy callback data because the ESP-IDF buffer is invalid after the callback returns.
static void queueReceivedFrame(const uint8_t* address, const uint8_t* data, size_t len) {
  ...
  if (rxLock.test_and_set(std::memory_order_acquire)) return; // callbacks never wait on the main loop
...
// ESP-IDF callback: defer all packet processing to WLED's main loop.
...
// ESP-IDF callback: publish completion without running application code here.
...
// Native ESP-NOW requires every unicast and broadcast destination to be a peer.
static bool checkForPeer(const uint8_t* address) {
  ...
  peer.channel = 0; // follow the active interface's home channel
  peer.ifidx = transportUsesAP ? WIFI_IF_AP : WIFI_IF_STA;
  peer.encrypt = false;
  return esp_now_add_peer(&peer) == ESP_OK;
}

// Move one queued callback frame into loop-owned stack storage.
...
// Allocate callback storage and attach ESP-NOW to the settled Wi-Fi interface.
...
// Disable callbacks before freeing memory they may access otherwise.
...
// Only one frame may be submitted until its completion callback is drained.
...
// Submit one frame, (zero means accepted, matching the legacy QuickESPNow convention).
...
// Deliver deferred send and receive callbacks from safe loop context.
void handleEspNowTransport() {
  ...
  // Clear the in-flight state before notifying clients so they may queue the next frame.
