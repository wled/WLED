...
// Transition durations are held in milliseconds as uint32_t.
// The upper limit is capped at 24h: this keeps every value within the signed 32 bit range used by the
// settings/HTTP APIs and stays far below the millis() rollover period (~49.7 days) that transition timing relies on.
constexpr uint32_t TRANSITION_MAX_DUR       = 86400000UL;             // maximum transition duration in ms
constexpr uint32_t TRANSITION_MAX_DUR_100MS = TRANSITION_MAX_DUR/100; // same limit in the 100ms units used by cfg.json, the JSON API and playlists

static_assert(TRANSITION_MAX_DUR <= INT32_MAX, "TRANSITION_MAX_DUR must fit into int: it is passed to printSetFormValue(..., int) in xml.cpp and parsed by getNumVal() (util.cpp), which returns int");
static_assert(TRANSITION_MAX_DUR % 100 == 0, "TRANSITION_MAX_DUR must be a whole number of 100ms units, otherwise TRANSITION_MAX_DUR_100MS would not describe the same limit");

// AI: below section was generated by an AI
// Calculates transition progress (0-0xFFFF) from the elapsed time and the total duration (both in ms).
// As durations may exceed 16 bits, both values are scaled down before multiplying so the intermediate
// result always stays within 32 bit math (the scaling is exact, no progress resolution is lost).
inline uint16_t transitionProgress(uint32_t elapsed, uint32_t duration) {
  if (elapsed >= duration) return 0xFFFFU;  // transition finished (also guards against duration == 0)
  if (duration > 0xFFFFU) {
    const unsigned shift = 32 - __builtin_clz(duration) - 16;  // number of bits by which the duration exceeds 16 bits (all supported MCUs are 32 bit)
    elapsed  >>= shift;
    duration >>= shift;
  }
...
class Segment {
  ...
  private:
    ...
    static uint32_t      _nextPaletteBlend;   // next due time for random palette morph (normalised millis(), see handleRandomPalette())
    static bool          _modeBlend;          // mode/effect blending semaphore
    // clipping rectangle used for blending
    static uint16_t      _clipStart, _clipStop;
    static uint8_t       _clipStartY, _clipStopY;

    // transition data, holds values during transition (80 bytes/32 bytes)
    struct Transition {
      ...
      uint32_t      _dur;                 // duration of transition in ms (max. TRANSITION_MAX_DUR)
  ...
  public:
    ...
    void startTransition(uint32_t dur, bool segmentCopy = true);    // transition has to start before actual segment values change
...
class WS2812FX {
  ...
  public:
    ...
    inline void setTransition(uint32_t t)                     { _transitionDur = min(t, TRANSITION_MAX_DUR); } // sets transition time (in ms)
    ...
    inline uint32_t getTransition() const   { return _transitionDur; }    // returns currently set transition time (in ms)
