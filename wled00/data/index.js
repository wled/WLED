		...
		if (ef.name.indexOf("RSVD") < 0) {
			...
			if (id !== 0) {
				if (_fxPresetList.find(p => p.effectId === parseInt(id, 10))) {
					btnHtml = `<i class="icons fx-preset-btn fx-preset-rm flr" onclick="event.stopPropagation();removeEffectPreset(${id})" title="Remove from cycle">&#xe38f;</i>`;
				} else {
					const safeName = nm.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
					btnHtml = `<i class="icons fx-preset-btn fx-preset-add flr" onclick="event.stopPropagation();addEffectAsPreset(${id},'${safeName}')" title="Add to cycle">&#xe18a;</i>`;
				}
			}
			// AI: end
...
function updateFxPresetBtn(effectId, effectName, isAdded) {
	...
	if (!listItem) return;
	const btn = listItem.querySelector('.fx-preset-btn');
	if (!btn) return;
	if (isAdded) {
		btn.className = 'icons fx-preset-btn fx-preset-rm flr';
		...
		btn.className = 'icons fx-preset-btn fx-preset-add flr';
...
async function addEffectAsPreset(effectId, effectName) {
	...
	showToast("Effect added to favorites!");
	...
	try {
		...
		pJson = presets;
		populatePresets();
		await requestJson({"ps": _FX_PLAYLIST_ID});
	} catch(e) {
...
async function removeEffectPreset(effectId) {
	...
	showToast("Effect removed from favorites!");
...
// AI: below section was generated by an AI
// Prevent iOS pull-to-refresh when swiping up from top of a scrollable area
var _touchY = 0;
...
document.addEventListener('touchmove', function(e) {
	...
	while (el && el !== document.documentElement) {
		if (el.scrollHeight > el.clientHeight) {
			if (el.scrollTop <= 0 && e.touches[0].clientY > _touchY) e.preventDefault();
			return;
		}
		el = el.parentElement;
	}
