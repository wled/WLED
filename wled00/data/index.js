...
var _FX_PRESETS_KEY = 'wled_user_fx_presets'; // localStorage key for user-added fx presets
// AI: end
var cfg = {
...
function onLoad()
{
	...
	(async ()=>{
		try {
			...
			await loadPresets();    // always load fresh so heart icons reflect actual device state
...
var timeout;
function showToast(text, error = false, duration = 3500, cls = '')
{
	var x = gId('toast');
	...
	x.innerHTML = text;
	x.classList.remove('show', 'show-green', 'error');
	x.style.animation = 'none';
	void x.offsetHeight;
	x.style.animation = '';
	x.classList.add(error ? 'error' : (cls || 'show'));
	clearTimeout(timeout);
	timeout = setTimeout(()=>{ x.classList.remove('show', 'show-green'); }, duration);
...
async function loadPresets() {
	return new Promise((resolve) => {
		...
		.then(json => {
			// Preserve any user-added fx-presets that are in memory but not yet
			// reflected in the server file (e.g. uploaded moments ago and the
			// server hasn't persisted yet, or the mock still serves a stale fixture).
			const existingFxPresets = getFxPresets();
			for (const entry of existingFxPresets) {
				const key = String(entry.presetId);
				if (!json[key] && pJson && pJson[key]) json[key] = pJson[key];
			}
			pJson = _deepCopyPresets(json);
			syncFxPresetsFromDevice();
			if (eJson && eJson.length > 0) populateEffects();
			populatePresets();
			resolve();
		})
		.catch(() => {
			// If we already have a cached preset list, re-render from it rather than
			// showing an error — the fetch may have raced with a concurrent upload.
			if (pJson && Object.keys(pJson).length > 0) {
				populatePresets();
			} else {
				presetError(false);
			}
...
{
	if (fromls) pJson = _deepCopyPresets(JSON.parse(localStorage.getItem("wledP")));
	...
	const _favsOrder = getFavsOrder();
	if (_favsOrder.length > 0) {
		arr.sort((a, b) => {
			const ia = _favsOrder.indexOf(parseInt(a[0]));
			const ib = _favsOrder.indexOf(parseInt(b[0]));
	...
	for (var idx = 0; idx < presetIds.length; idx++)
	{
		...
		const fxEntry = fxList.find(p => p.presetId === i);
		const fxBtnHtml = fxEntry
	...
	if (pNum === 0) {
		gId('pcont').innerHTML = '<p id="noFavsMsg" style="text-align:center;color:#888;padding:32px 16px 0;margin:0;font-size:15px">You have no presets saved</p>';
		gId('pql').style.display = 'none';
		var ph = gId('presetsHeading'); if (ph) ph.style.display = 'none';
		return;
	}
	// Restore heading and quick-links visibility for non-empty state
	var ph2 = gId('presetsHeading'); if (ph2) ph2.style.display = '';
...
function populateEffects()
{
	...
	// AI: below section was generated by an AI
	const _fxPresetList = getFxPresets(); // cache once for the loop below
	// AI: end
	for (let ef of effects) {
		...
		if (ef.name.indexOf("RSVD") < 0) {
			...
			// AI: below section was generated by an AI
			let btnHtml = '';
			if (_fxPresetList.find(p => p.effectId === parseInt(id, 10))) {
				btnHtml = `<span class="fx-preset-btn fx-preset-rm" onclick="event.stopPropagation();removeEffectPreset(${id})" title="Remove from cycle">&#x2605;</span>`;
			} else {
				const safeName = nm.replace(/\\/g,'\\\\').replace(/'/g,"\\'");
				btnHtml = `<span class="fx-preset-btn fx-preset-add" onclick="event.stopPropagation();addEffectAsPreset(${id},'${safeName}')" title="Add to cycle">&#x2606;</span>`;
			}
			// AI: end
			html += generateListItemHtml('fx',id,nm,'setFX',btnHtml,fd);
		}
	}
	...
	for (let row of gId('pallist').querySelectorAll('.fx-pill-row')) {
		...
		if (shouldHide) {
			...
			updateBtn.addEventListener('click', (e) => {
				updateCurrentPresetPalette(pal.dataset.id);
				e.stopPropagation();
			});
		}
	}
...
{
	const pill = `<div class="lstI${id==0?' sticky':''}" data-id="${id}" ${effectPar===''?'':'data-opt="'+effectPar+'" '}onClick="${clickAction}(${id})">`+
	...
	`</div>`;
	if (extraHtml) {
		return `<div class="fx-pill-row">${pill}${extraHtml}</div>`;
	}
	return pill;
}
...
function setFX(ind = null)
{
	...
	// AI: below section was generated by an AI
	// If this effect has a starred preset, load the preset so saved slider values are restored.
	var _fxPresetEntry = getFxPresets().find(function(p) { return p.effectId === parseInt(ind); });
	if (_fxPresetEntry && pJson[_fxPresetEntry.presetId] && !pJson[_fxPresetEntry.presetId].win) {
		setPreset(_fxPresetEntry.presetId);
		return;
	}
	// AI: end
...
// AI: below section was generated by an AI
function _schedulePresetAutoSave()
{
	// Determine preset to save to. Skip WIN-macro presets (e.g. z_cycle_preset ID 101) —
	// saving to them would overwrite the cycling playlist with raw effect data.
	var pid = currentPreset;
	if (pid <= 0 || !pJson[pid] || pJson[pid].win) {
		// Fall back: if the currently-playing effect has a starred preset, save there.
		var fxEntry = getFxPresets().find(function(p) { return p.effectId === selectedFx; });
		if (!fxEntry || !pJson[fxEntry.presetId] || pJson[fxEntry.presetId].win) return;
		pid = fxEntry.presetId;
	}
	// Capture slider values NOW (at schedule time). The z_cycle_preset continuously
	// re-applies the saved preset from disk, so readState() can reset the DOM sliders
	// before the 500ms timer fires. Capturing here preserves the user's intended values.
	var sx = parseInt(gId('sliderSpeed').value);
	var ix = parseInt(gId('sliderIntensity').value);
	var c1 = parseInt(gId('sliderC1').value) || 0;
	var c2 = parseInt(gId('sliderC2').value) || 0;
	var c3 = parseInt(gId('sliderC3').value) || 0;
	clearTimeout(_presetSaveTimer);
	_presetSaveTimer = setTimeout(async function() {
		if (!pJson[pid] || pJson[pid].win) return;
		// Re-apply the captured values so the device state is current, then save.
		// Two sequential requests via WebSocket are processed in order by the device,
		// so the save captures the re-applied values rather than a cycle-reset state.
		await requestJson({seg: {sx: sx, ix: ix, c1: c1, c2: c2, c3: c3}});
		if (!pJson[pid] || pJson[pid].win) return;
		await requestJson({psave: pid, n: pJson[pid].n || ''});
		// Keep pJson in sync so setPreset in the same session sends correct slider values.
		if (!pJson[pid] || !pJson[pid].seg || !pJson[pid].seg[0]) return;
		pJson[pid].seg[0].sx = sx;
		pJson[pid].seg[0].ix = ix;
		pJson[pid].seg[0].c1 = c1;
		pJson[pid].seg[0].c2 = c2;
		pJson[pid].seg[0].c3 = c3;
	}, 500);
}
// AI: end
...
function updateCurrentPresetPalette(paletteId) {
	...
	if (!pJson[currentPreset].seg || !pJson[currentPreset].seg[0]) {
		showToast("Preset structure invalid.", true);
		return;
	}

	// Update the palette in memory
	pJson[currentPreset].seg[0].pal = paletteId;

	// Save the preset to device
	const presetToSave = currentPreset;
	const presetObj = _deepCopyPresets({[String(presetToSave)]: pJson[presetToSave]});
	const presetsToUpload = _deepCopyPresets(pJson);
	presetsToUpload[String(presetToSave)] = presetObj[String(presetToSave)];

	_uploadPresetsJson(presetsToUpload).then(() => {
		pJson = presetsToUpload;
		showToast("Favourite color updated", false, 2000, 'show-green');
	}).catch(err => {
		showToast("Failed to update preset: " + err.message, true);
	});
}
...
function filterFx() {
	...
	gId("fxlist").querySelectorAll('.lstI').forEach((listItem, i) => {
		...
		const target = listItem.closest('.fx-pill-row') || listItem;
		target.style.display = hide && !listItem.classList.contains("selected") ? 'none' : '';
	});
}
...
// AI: below section was generated by an AI
// User fx preset management — lets users add/remove effects from the auto-cycling playlist
...
function _deepCopyPresets(presets) {
	// Deep copy all presets to break any shared references that could cause
	// modifications to one preset to affect others (especially with Default palette)
...
function saveFxPresets(list) {
	localStorage.setItem(_FX_PRESETS_KEY, JSON.stringify(list));
}

// AI: below section was generated by an AI
function getFavsOrder() {
	try { return JSON.parse(localStorage.getItem('wledFavsOrder') || '[]'); } catch(e) { return []; }
}

function saveFavsOrderFromDom() {
	const rows = [...gId('pcont').querySelectorAll('.fx-pill-row[data-pid]')];
	localStorage.setItem('wledFavsOrder', JSON.stringify(rows.map(r => parseInt(r.dataset.pid))));
}
...
function getFxPresetByEffect(effectId) {
	return getFxPresets().find(p => p.effectId === effectId) || null;
}

function updateFxPresetBtn(effectId, effectName, isAdded) {
	const listItem = document.querySelector(`#fxlist .lstI[data-id="${effectId}"]`);
	if (!listItem) return;
	const btn = listItem.parentElement ? listItem.parentElement.querySelector('.fx-preset-btn') : null;
	if (!btn) return;
	if (isAdded) {
		btn.className = 'fx-preset-btn fx-preset-rm';
		btn.setAttribute('title', 'Remove from cycle');
		btn.setAttribute('onclick', `event.stopPropagation();removeEffectPreset(${effectId})`);
		btn.innerHTML = '&#x2605;';
	} else {
		const safeName = effectName.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
		btn.className = 'fx-preset-btn fx-preset-add';
		btn.setAttribute('title', 'Add to cycle');
		btn.setAttribute('onclick', `event.stopPropagation();addEffectAsPreset(${effectId},'${safeName}')`);
		btn.innerHTML = '&#x2606;';
	}
}

async function _uploadPresetsJson(presetsObj) {
	const formData = new FormData();
	const blob = new Blob([JSON.stringify(presetsObj)], {type: 'application/json'});
	formData.append('data', blob, '/presets.json');
	const resp = await fetch(getURL('/upload'), {method: 'POST', body: formData});
	if (!resp.ok) throw new Error('Upload failed: ' + resp.status);
}

async function addEffectAsPreset(effectId, effectName) {
	const list = getFxPresets();
	if (list.find(p => p.effectId === effectId)) return;
	const nextId = list.length > 0 ? Math.max(...list.map(p => p.presetId)) + 1 : _FX_PRESETS_BASE + 1;

	// Capture slider and palette values synchronously before any async operation — reading them
	// after an await risks stale values if the user navigates to another effect first.
	const sx = parseInt(gId('sliderSpeed').value);
	const ix = parseInt(gId('sliderIntensity').value);
	const c1 = parseInt(gId('sliderC1').value);
	const c2 = parseInt(gId('sliderC2').value);
	const c3 = parseInt(gId('sliderC3').value);
	const palInput = d.querySelector('#pallist input[name="palette"]:checked');
	let pal = palInput ? parseInt(palInput.value) : 0;
	// Avoid palette 0 (Default) - it doesn't preserve custom color values (c1, c2, c3)
	// Use palette 6 (Party) as fallback which supports custom colors properly
	if (pal === 0) pal = 6;

	list.push({effectId, presetId: nextId, effectName});
	saveFxPresets(list);
	updateFxPresetBtn(effectId, effectName, true);
	showToast("Effect added to presets!", false, 1000, 'show-green');

	try {
		// Use in-memory pJson as the source of truth to avoid a race with loadPresets()
		// that could be fetching the same file concurrently during page init.
		const presets = JSON.parse(JSON.stringify(pJson || {}));
		...
		const i = list.findIndex(p => p.effectId === effectId);
		if (i >= 0) list.splice(i, 1);
		saveFxPresets(list);
		updateFxPresetBtn(effectId, effectName, false);
		showToast("Save failed: " + e.message, true);
	}
}

async function removeEffectPreset(effectId) {
	const list = getFxPresets();
	const idx = list.findIndex(p => p.effectId === effectId);
	if (idx < 0) return;
	const entry = list[idx];
	const lastIdx = list.length - 1;
	const removedName = entry.effectName;
	const swapOccurs = idx < lastIdx;
	const lastEntry = swapOccurs ? Object.assign({}, list[lastIdx]) : null;

	if (swapOccurs) {
		list[idx] = {effectId: lastEntry.effectId, presetId: entry.presetId, effectName: lastEntry.effectName};
	}
	list.splice(lastIdx, 1);
	saveFxPresets(list);
	updateFxPresetBtn(effectId, removedName, false);
	showToast("Effect removed from presets!", false, 1000, 'show-green');
...
function syncFxPresetsFromDevice() {
	...
	for (const [key, preset] of Object.entries(pJson)) {
		...
		synced.push({ effectId, presetId: numId, effectName: preset.n || '' });
	}
	// Only replace localStorage when the device actually has fx-presets to offer.
	// If the device returns an empty set (e.g. the upload hasn't propagated yet or
	// the mock still serves the original fixture) keep whatever is already stored so
	// we don't wipe entries that were just added in this session.
	if (synced.length > 0 || getFxPresets().length === 0) {
		saveFxPresets(synced);
	}
}
// AI: end
...
// Prevent pull-to-refresh without blocking scroll-up gestures.
// CSS overscroll-behavior:none alone is insufficient when body is position:fixed —
// Chrome Android still triggers pull-to-refresh from inner scroll containers.
// We record scrollTop at touchstart; if the gesture started at the top (0) and the
// finger moves down, nothing above exists to scroll to, so we block the event.
// If the gesture started scrolled down, we allow the whole gesture (including
...
(function() {
	...
	document.addEventListener('touchstart', function(e) {
		...
		while (el && el !== document.documentElement) {
			if (el.scrollHeight > el.clientHeight + 1) { startScrollTop = el.scrollTop; break; }
			el = el.parentElement;
		}
	}, { passive: true });
