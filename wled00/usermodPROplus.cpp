#ifndef USERMOD_ID
#define USERMOD_ID 100
#endif

#include <Arduino.h>
#include "usermodPROplus.h"
#include <WLED.h>
...
// Deklaracje zewnętrzne wymagane przez WLED
extern uint8_t effectCurrent;
extern struct {
  uint16_t hue;
} state;
extern void stateUpdated(uint8_t mode);
extern const uint8_t CALL_MODE_USER;
extern bool WLED_MQTT_CONNECTED;
extern AsyncMqttClient* mqtt;
extern NmUI nmUI; // jeśli taki obiekt istnieje w projekcie


class UsermodPROplus : public Usermod {
  public:
    ...
    void setup() {
      ...
      if (mqttEnabled && WLED_MQTT_CONNECTED) mqtt->subscribe(mqttTopicCmd.c_str(), 0);
    }
    void loop() {
      uint32_t now = millis();
  ...
  private:
    ...
    void sendEvents(uint8_t p) {
      if (mqttEnabled && WLED_MQTT_CONNECTED) {
        ...
        mqtt->publish(mqttTopicCmd.c_str(), 0, true, b, n); // poprawione wywołanie publish
      }
    }
